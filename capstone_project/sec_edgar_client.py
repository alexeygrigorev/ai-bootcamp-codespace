...
try:
    from sec_xml_parser import parse_sec_filing as _parse_sec_filing, chunk_sec_documents as _chunk_sec_documents, SECXMLParser as _SECXMLParser
...
class SECEdgarClient:
    ...
    def download_filing_document(self, accession_number: str, primary_document: str, 
        ...
        """
        # Convert accession number to directory path
        accession_num_with_dashes = accession_number.replace('-', '')
        
        # Construct document URL - try both with and without dashes
        doc_url = f"{self.BASE_URL}/files/edgar/data/{cik}/{accession_num_with_dashes}/{primary_document}"
        
        try:
            response = requests.get(doc_url, headers=self.headers, timeout=30)
            
            # If not found, try alternative URL patterns
            if response.status_code == 404:
                # Try with dashes in accession number
                alternate_url = f"{self.BASE_URL}/files/edgar/data/{cik}/{accession_number}/{primary_document}"
                response = requests.get(alternate_url, headers=self.headers, timeout=30)
                
                # If still not found, try XBRL format
                if response.status_code == 404:
                    alternate_url = f"{self.BASE_URL}/submissions/{accession_number}/{primary_document}"
                    response = requests.get(alternate_url, headers=self.headers, timeout=30)
